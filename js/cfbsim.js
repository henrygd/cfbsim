...
(function() {
  ...
  var cfbSim = global.CFBSim = {
    ...
    loadURLTeams: function() {
      var urlTeams = global.location.hash.replace('#', '').split('_vs_');
    ...
    startSim: function(){
      ...
      ShowStats = setTimeout(function(){
        ...
        var simBar = $('#sim_bar');
        if (statsHeight + simBar.height() + $('#close_results_btn').height() > $(global).height()) {
  ...
  cfbSim.compareStrengths = (function() {
    ...
    return function() {
      clearTimeout(global.TimeoutCompareStrengths);
      // timout to prevent two calls on simultaneous switch of both teams
      global.TimeoutCompareStrengths = setTimeout(function() {
        var teamOneName = cfbSim.curTeams[0].text();
        var teamTwoName = cfbSim.curTeams[1].text();
        var teamOne = new Team(teamOneName);
        var teamTwo = new Team(teamTwoName);
        ...
        // set global location to match teams
